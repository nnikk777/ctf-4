...
Для получения флага, находящегося в директории проекта, можно изменить темлпейт эксплойта следующим образом:
```
return (os.system, ('python -m http.server 5001',))

```
После создать куки, вставить ее вместо оригинальных и перезагрузить страницу. В таком случае контейнер создаст на порту 5001 http 
сервер в текущей директории проекта. Там можно найти содержимое флага.
